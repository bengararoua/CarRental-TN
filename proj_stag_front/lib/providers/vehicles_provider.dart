...
class VehiclesProvider with ChangeNotifier {
  ...
  //Retourne l’email de l’utilisateur si existe, sinon null
  //_user; // accessible UNIQUEMENT dans vehicles_provider.dart
  ...
  //Map est une structure de données qui permet d’associer une clé à une valeur pour stocker et accéder facilement aux données.
  ...
  //.toList():retourne le résultat sous forme de liste
  ...
  // Future représente une valeur qui sera disponible plus tard
  ...
  Future<void> toggleFavorite(int vehicleId) async {
    ...
    try {
      ...
      if (isCurrentlyFav) {
        // Si le véhicule était déjà favori(=>true), le retirer des favoris côté serveur
